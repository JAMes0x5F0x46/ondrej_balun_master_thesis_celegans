...
% these neurons.  
function [output] = mainTWSimulation_neuron_ablation(stimID, knockedOut)
%Input: 
...
pulse_dur = 0.03; % stimulation duration 40ms
...
output_avb = store_V(:,AVB_ID);
output_ava = store_V(:,AVA_ID);
...
% plot(store_t*1000, store_V(:,3)*1000,'g','linewidth', 2);
% plot(store_t*1000, store_V(:,1)*1000,'black','linewidth', 2);
% plot(store_t*1000, store_V(:,8)*1000,'y','linewidth', 2);
legend('AVA','AVB');
xlabel('Time (ms)');
ylabel('Potential (mV)');
