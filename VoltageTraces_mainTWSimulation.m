...
% these neurons.  
function [output] = VoltageTraces_mainTWSimulation(stimID, knockedOut)
%Input: 
...
output_a = sum((store_V(startInd:endInd,AVB_ID) - ...
    store_V(startInd:endInd,AVA_ID)))*(startTime+pulse_dur);

output_avb = store_V(startInd:endInd,AVB_ID);
output_ava = store_V(startInd:endInd,AVA_ID);
...
%output_f = forward * 0.1; 
%output_b= backward * 0.1; 
...
% % plot(store_t*1000, store_V(:,4)*1000,'g','linewidth', 2);
% % plot(store_t*1000, store_V(:,6)*1000,'black','linewidth', 2);
% % plot(store_t*1000, store_V(:,9)*1000,'y','linewidth', 2);
% legend('AVA','AVB');
% xlabel('Time (ms)');
% ylabel('Potential (mV)');
...
output = [store_t * 1000, store_V(:,AVA_ID) * 1000, store_V(:,AVB_ID) * 1000, store_V(:,1) * 1000, store_V(:,3) * 1000, store_V(:,4) * 1000, store_V(:,6) * 1000, store_V(:,9) * 1000];
end
